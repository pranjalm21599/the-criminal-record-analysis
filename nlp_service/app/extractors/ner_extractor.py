...
class NERExtractor:
    ...
    def __init__(self):
        # A missing model must not take the whole service down. Fall back to a
        # blank English pipeline: statistical NER (persons, locations, orgs)
        # goes quiet, but sentence handling and every regex-based extractor
        # keep working, so uploads still produce phones, vehicles and accounts.
        try:
            self.nlp = spacy.load("en_core_web_sm")
            self.model_loaded = True
        except OSError:
            logger.warning(
                "spaCy model 'en_core_web_sm' not installed — named-entity "
                "extraction is disabled. Install it with: "
